...
while True:
    if game_state == "start":
        ...
        if not level:
            ...
            sys.exit()
        if level == 1:
            sound_manager.play_music("assets/mp3/level1.mp3")
            with open("levels/level1.json", "r") as f:
                events = json.load(f)
        elif level == 2:
            sound_manager.play_music("assets/mp3/level2.mp3")
        ...
        if not level_initialized:
            ...
            # 音樂與事件載入
            if level == 1:
                sound_manager.play_music("assets/mp3/level1.mp3", start_time=40.94 + time_skip, fade_ms=1000, loop = 0)
                bpm_scale = bpm_scale1
                sound_manager.set_volume(0.45)
            elif level == 2:
                sound_manager.play_music("assets/mp3/level2.mp3", start_time=15.45 + time_skip, fade_ms=1000, loop = 0)
